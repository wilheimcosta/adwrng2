...
export default async function handler(request: ApiRequest, response: ApiResponse) {
  ...
  try {
    const upstream = await fetchWithTimeout(`https://aviationweather.gov/api/data/metar?ids=${encodeURIComponent(icao)}&format=json`, { headers: { Accept: "application/json" } }, 8_000);
